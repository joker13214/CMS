	...
	, m_bFirstCfg(true)
{
	...
	m_nAppProVer = 1;
	m_nScFactoryType = SCFactoryType_GTDZ_4_0;
	m_nScAddress = 0;
}
...
BOOL CUdfAddRtuStDlg::OnInitDialog()
{
	...
	if (m_wndCmbScFactory.GetSafeHwnd())
	{
		...
		int Idx = 0, nSel = -1;
		Idx = m_wndCmbScFactory.AddString(CUdfGlobalFUNC::GetSCFactoryString(SCFactoryType_GTDZ_4_0));
		m_wndCmbScFactory.SetItemData(Idx,SCFactoryType_GTDZ_4_0);
		if (m_nScFactoryType == SCFactoryType_GTDZ_4_0)
			nSel = Idx;
