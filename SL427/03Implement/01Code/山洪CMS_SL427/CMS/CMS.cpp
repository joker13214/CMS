...
BOOL CCMSApp::LoadConfigFile()
{
	...
	if (!blOptExist)
	{
		m_sysConfigObj.AddOneOption("APP_TITLE","(山洪CMS_SL427)CMS - 通信前置机","",TRUE);
		m_GlobalOptions.m_strAppTitie = "(山洪CMS_SL427)CMS - 通信前置机";
	}
	...
	if (!blOptExist)
	{
		m_sysConfigObj.AddOneOption("DATABASE_TYPE","1","数据库类型",TRUE);
