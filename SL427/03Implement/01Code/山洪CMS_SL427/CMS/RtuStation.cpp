...
CRtuStation::CRtuStation(void)
{
	...
	m_nScFactoyType = SCFactoryType_GTDZ_4_0;
	m_nScAddress = 0;
...
void CRtuStation::ClearDisplaceLimitParaList()
{
	while (!m_lstDisplaceLimitParas.IsEmpty())
	{
		CDisplace_PARA_PER* pDisplace_PARA = m_lstDisplaceLimitParas.RemoveHead();
		if (pDisplace_PARA)
		{
			delete pDisplace_PARA;
			pDisplace_PARA = NULL;
		}
	}
}

void CRtuStation::ClearTemperaLimitParaList()
{
	while (!m_lstTemperaLimitParas.IsEmpty())
	{
		CTempera_PARA_PER* pTempera_PARA = m_lstTemperaLimitParas.RemoveHead();
		if (pTempera_PARA)
		{
			delete pTempera_PARA;
			pTempera_PARA = NULL;
		}
	}
}
...
void CRtuStation::ClearTmUnitList()
{
	std::list<TM_Unit*>::iterator it;
	TM_Unit *p = NULL;
	for (it=m_lstTmUnit.begin();
		it!=m_lstTmUnit.end();
		it++)
	{
		p = (TM_Unit *)(*it);
...
void CRtuStation::Clear()
{
	...
	ClearDisplaceLimitParaList();
	ClearTemperaLimitParaList();
...
bool CRtuStation::BuildExtInformation(CDatabase* pDb)
{
	...
	try
	{
		sSQL.Format("SELECT pVer,scAddr,scFactroyType,WorkMode FROM rtuStation_ext WHERE sID='%s'",m_sID);
		CRecordset rs(pDb);
		...
		if (blEmpty)
		{
			sSQL.Format("INSERT INTO rtuStation_ext(sID,pVer,scAddr,scFactroyType,WorkMode) VALUES ('%s',%d,%d,%d,%d)",
				m_sID,m_nAppProVer,m_nScAddress,m_nScFactoyType,m_RtuWorkMode);
...
bool CRtuStation::BuildExt_Para(CDatabase* pDb)
{
	bool _blResult = true;
	...
	try
	{
		...
		*/
		...
		sSQL.Format("SELECT p_Value FROM rtu_ext_para WHERE sID='%s' AND p_Name='DZ_DELAY'",m_sID);
		CRecordset rs(pDb);
		...
		if (!rs.IsEOF())
		{
			rs.GetFieldValue("p_Value",strTemp);
			m_nDzDelay = atoi(strTemp);
		...
		if (blEmpty)
		{
			sSQL.Format("INSERT INTO rtu_ext_para(sID,p_Name,p_Value) VALUES ('%s','DZ_DELAY','%d')",
				m_sID,m_nDzDelay);
	...
	catch (CDBException* pEx)
	{
		TRACE("%s\r\n",pEx->m_strError);
		pEx->Delete();
		GETAPP()->m_pReadDB->CreatertuStation_extParaTable();
...
bool CRtuStation::BuildBasicPacket(CSmsFrameFormat& appFrame)
{
	appFrame.m_CommInterface		= GetInterface();
	appFrame.m_NetInfo.mIpAddress	= GetIpAddrA();
	appFrame.m_NetInfo.mPort		= GetSockPortA();
...
CDisplace_PARA_PER* CRtuStation::GetDisplaceLimitPara(int Index)
{
	POSITION rPos = m_lstDisplaceLimitParas.FindIndex(Index);
	if (rPos == NULL)
		return NULL;

	return m_lstDisplaceLimitParas.GetAt(rPos);
}

CTempera_PARA_PER* CRtuStation::GetTemperaLimitPara(int Index)
{
	POSITION rPos = m_lstTemperaLimitParas.FindIndex(Index);
	if (rPos == NULL)
		return NULL;

	return m_lstTemperaLimitParas.GetAt(rPos);
}
...
TM_Unit* CRtuStation::GetTmUnitFromMap(int idx)
{
	std::list<TM_Unit*>::iterator it;
...
void CRtuStation::SetTmMessageDealModule(CTmMessageDealModule* pTmMessageModule)
{
	m_cs_TmMessageDealModule.Lock();
	if (m_pTmMessageDealModule != pTmMessageModule)
		m_pTmMessageDealModule = pTmMessageModule;
	m_cs_TmMessageDealModule.Unlock();
}
...
CTmMessageDealModule* CRtuStation::GetTmMessageDealModule()
{
	return m_pTmMessageDealModule;
}
...
bool CRtuStation::SendData(BYTE* buffer,WORD len)
{
	...
	else if (m_CmType == TEXTMSG_CommChannel)
	{
		m_cs_TmMessageDealModule.Lock();
		if (m_pTmMessageDealModule != NULL)
		{
			m_pTmMessageDealModule->SendMessage(buffer,len);
		}
		m_cs_TmMessageDealModule.Unlock();
	}
	else 
		...
		if (m_Interface == Ethernet_CommInerface)   //网络
		{
			...
			else if (TCP_Protocol == m_ProtocolType) //TCP方式
			{
				...
				if (m_pHandle_TcpClient != NULL)
				{
					...
					m_pHandle_TcpClient->AddSendMessage(buffer,len);
				}
				m_cs_HandleTcpClient.Unlock();
			}
		}
...
void  CRtuStation::SetTmCommInfo(PSC_TEXTMSG_INFO pInfo)
{
	if (pInfo != NULL)
	{
		SetTmAddress(pInfo->nTmAddress);
		SetTmFactoryType(pInfo->nFactoryType);
	}
}
...
CDisplace_PARA_PER::CDisplace_PARA_PER( CDisplace_PARA_PER* pSource )
{
	if (pSource != NULL)
	{
		this->m_Displace_XUpLimit = pSource->GetDisplaceXUpLimit();
		this->m_Displace_XDownLimit = pSource->GetDisplaceXDownLimit();
		this->m_RealTimeXValue = pSource->GetRealTimeXValue();
		this->m_Displace_YUpLimit = pSource->GetDisplaceYUpLimit();
		this->m_Displace_YDownLimit = pSource->GetDisplaceYDownLimit();
		this->m_RealTimeYValue = pSource->GetRealTimeYValue();
		this->m_Displace_No = pSource->GetDisplaceNo();
	}
	else
	{
		CDisplace_PARA_PER();
...
CTempera_PARA_PER::CTempera_PARA_PER( CTempera_PARA_PER* pSource )
{
	if (pSource != NULL)
	{
		this->m_Tempera_DownLimit = pSource->GetTemperaDownLimit();
		this->m_Tempera_UpLimit = pSource->GetTemperaUpLimit();
		this->m_RealTimeValue = pSource->GetRealTimeValue();
	}
	else
	{
