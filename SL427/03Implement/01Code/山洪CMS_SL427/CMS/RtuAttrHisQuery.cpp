...
void CRtuAttrHisQuery::OnBnClickedButton1()
{
	...
	if (pApp->m_pRtuDatasave ==NULL)
		return;
	if (pApp->m_pRtuDatasave->m_pDataTranSQLiteLocal == NULL)
		return;
	...
	try
	{
		...
		if(m_sBegin !="" && m_sEnd != "")
		{
			...
			if (pApp->m_pRtuDatasave->m_pDataTranSQLiteLocal->m_blStartTransaction == false)
			{
				pApp->m_pRtuDatasave->m_pDataTranSQLiteLocal->SQLiteCS_Lock();
				blLocked = true;
				CSqlStatement* rs = pApp->m_pRtuDatasave->m_pDataTranSQLiteLocal->m_sqlite.Statement(sSQL);
				if (rs != NULL)
				{
					InitGridHeader();
					while (rs->NextRow())
				...
				pApp->m_pRtuDatasave->m_pDataTranSQLiteLocal->SQLiteCS_UnLock();
				blLocked = false;
			}
		...
		/*
		//20140811，修改SQLite操作，采用全局控制只操作一个SQLite文件
