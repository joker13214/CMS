...
void CCommThreadInfo::SetTmCommInfo(PSC_TEXTMSG_INFO pInfo)
{
	if (pInfo)
	{
		m_stTmCommInfo.nTmAddress = pInfo->nTmAddress;
		m_stTmCommInfo.nFactoryType  = pInfo->nFactoryType;
	}
}
...
unsigned long CCommServer::CommThreadFunction(void* pPara)
{
	...
	else if (bComInterfaceType == Serial_CommInterface)	//COM 串口通信
	{
		...
		if (bCommType == SC_CommChannel)  //北斗卫星
		{
			if (pModule->GetScCommInfo()->nFactoryType == SCFactoryType_GTDZ_4_0)  //国腾电子V4.0版协议
			{
				pScModule = new CScModule_GTDZ();
				pScModule->SetCommThreadInfo(pModule);
				pScModule->SetScCommInfo(pModule->GetScCommInfo());
			}
			else if (pModule->GetScCommInfo()->nFactoryType == SCFactoryType_GTDZ_2_5) //国腾电子V2.5版协议
			{
				pScModule = new CScModule_GTDZ_2_5();
				pScModule->SetCommThreadInfo(pModule);
				pScModule->SetScCommInfo(pModule->GetScCommInfo());
			}
		}
		else if (bCommType == TEXTMSG_CommChannel) //短信
		{
			if (pModule->GetTmCommInfo()->nFactoryType == SCTextMsgType_GHT)
			{
				pScModule = new TextMsg_GHT();
				TextMsg_GHT *ght = static_cast<TextMsg_GHT*>(pScModule);
				ght->SetCommInfo(*pComInfo);
				pScModule->SetCommThreadInfo(pModule);
				pScModule->SetTmCommInfo(pModule->GetTmCommInfo());

			}
			else if (pModule->GetTmCommInfo()->nFactoryType == SCTextMsgType_YT)
			{
				pScModule = new TextMsg_YT();
				TextMsg_YT *yt = static_cast<TextMsg_YT*>(pScModule);
				yt->SetCommInfo(*pComInfo);
				pScModule->SetCommThreadInfo(pModule);
				pScModule->SetTmCommInfo(pModule->GetTmCommInfo());
			}
		}
			...
			bCommType != SC_CommChannel)										//以太网 
		{
			...
			else if ( TCP_Protocol == bProtocolType)							//TCP通信方式
			{
				pHandleTcpServer->DealConnectionEvent();
			     ...
			     && bCommType != SC_CommChannel
				 && bCommType != TEXTMSG_CommChannel)						        //串口非卫星通信
		{
			...
			bCommType == TEXTMSG_CommChannel)
		{
			long len = pModule->GetHandle_Com()->lookCOMBuf();       //串口短信通讯
			len = len<READPORT_LEN_PER ?len : READPORT_LEN_PER;
	...
	try
	{
		...
		while (file.ReadString(strLine))
		{
			...
			if (pNewThread)
			{
				...
				SC_TEXTMSG_INFO Tm_comm_info;
				Tm_comm_info.nTmAddress = TmAddrLocal;
				Tm_comm_info.nFactoryType = TmFactory;
				pNewThread->SetTmCommInfo(&Tm_comm_info);
...
bool CCommServer::SaveThreadsCfg()
{
	...
	try
	{
		...
		CString strLine = "#THRADID=CMTYPE,INTERFACETYPE,PROTOCOL,IP,PORT,COMNAME,BAUND,DATABITS,STOPBITS,PARITY,SCFACTORY,SCADDR,TMFACTORY,TMADDR\n";
		...
		while (rPos != NULL)
		{
			...
			if (pThread)
			{
				strLine.Format("%s=%d,%d,%d,%s,%d,%s,%d,%d,%d,%d,%d,%d,%d,%d\n",
					...
					pThread->GetTmCommInfo()->nFactoryType,
					pThread->GetTmCommInfo()->nTmAddress);
				_fp.WriteString(strLine);
			}
		}
		_fp.Flush();
