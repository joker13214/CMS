	...
	: CDialog(CRtuAttrProSet::IDD, pParent)
{
	...
	m_pTabAttrTemperature = NULL;
	m_pTabAttrDisplacement = NULL;
...
BOOL CRtuAttrProSet::OnInitDialog()
{
	...
	if (GETAPP()->m_GlobalOptions.m_nWaterYieldEnable == 1)
	{
	...
	if (GETAPP()->m_GlobalOptions.m_nWaterLevelEnable == 1)
	{
	...
	if (GETAPP()->m_GlobalOptions.m_nWaterPressurelEnable == 1)
	{
	...
	if (GETAPP()->m_GlobalOptions.m_nWaterQualityEnable == 1)
	{
	...
	if (GETAPP()->m_GlobalOptions.m_nFlowEnable == 1)
	{
	...
	if (GETAPP()->m_GlobalOptions.m_nRainGaugeEnable == 1)
	{
	...
	if (GETAPP()->m_GlobalOptions.m_nAnalogEnable == 1)
	{
	...
	if (GETAPP()->m_GlobalOptions.m_nDispalcementEnable == 1)
	{
		m_pTabAttrDisplacement = new CRtuAttrDisplacement(&m_wndProTabCtrl);
		m_pTabAttrDisplacement->SetParentDlg(m_pParentDlg);
		m_pTabAttrDisplacement->Create(CRtuAttrDisplacement::IDD,&m_wndProTabCtrl);
		m_wndProTabCtrl.AddTab(m_pTabAttrDisplacement,"深度位移量");
	}

	if (GETAPP()->m_GlobalOptions.m_nElectricityEnable == 1)
	{
	...
	if (GETAPP()->m_GlobalOptions.m_nTemperatureEnable == 1)
	{
		m_pTabAttrTemperature = new CRtuAttrTemperature(&m_wndProTabCtrl);
		m_pTabAttrTemperature->SetParentDlg(m_pParentDlg);
		m_pTabAttrTemperature->Create(CRtuAttrTemperature::IDD,&m_wndProTabCtrl);
		m_wndProTabCtrl.AddTab(m_pTabAttrTemperature,"温度");
	}
...
void CRtuAttrProSet::Update()
{
	...
	if (m_pTabAttrDisplacement && m_pTabAttrDisplacement->GetSafeHwnd())
		m_pTabAttrDisplacement->Update();

	if (m_pTabAttrTemperature && m_pTabAttrTemperature->GetSafeHwnd())
		m_pTabAttrTemperature->Update();
...
void CRtuAttrProSet::OnDestroy()
{
	...
	if (m_pTabAttrDisplacement != NULL)
		delete m_pTabAttrDisplacement;

	if (m_pTabAttrTemperature != NULL)
		delete m_pTabAttrTemperature;
