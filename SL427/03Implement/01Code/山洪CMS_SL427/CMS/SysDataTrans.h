...
public:
	//短信版本增加的设置
	void IEC_textMsg_cpms(CSmsFrameFormat* tmFrame);
	void IEC_textMsg_cpmsa(CSmsFrameFormat* tmFrame);
	void IEC_textMsg_cmgf(CSmsFrameFormat* tmrFrame);
	void IEC_textMsg_csmp(CSmsFrameFormat* tmFrame);
	void IEC_textMsg_cmgl(CSmsFrameFormat* tmFrame);
	void IEC_textMsg_cmgr(CSmsFrameFormat* tmFrame);
	void IEC_textMsg_cmgd(CSmsFrameFormat* tmFrame);
...
protected:
	...
	void IEC_waterdata_US_6AH(CPacketFrameFormat* waterFrame);   //查询遥测站深度位移量数据上下限（ANF＝6AH）//add by yhp20160614
	void IEC_waterdata_US_6BH(CPacketFrameFormat* waterFrame);   //查询遥测站温度量数据上下限（ANF＝6BH）//add by yhp20160614
...
public:
   ...
   void IEC_waterdata_EN_21H(CPacketFrameFormat* appFrame);		//设置遥测终端深度位移量上下限值//add by yhp20160614
   void IEC_waterdata_EN_22H(CPacketFrameFormat* appFrame);		//设置遥测终端温度量上下限值//add by yhp20160614
   //21H-4FH  备用
   ...
   void IEC_waterdata_EN_6AH(CPacketFrameFormat* appFrame);		//查询遥测终端深度位移量上下限值 //add by yhp20160614
   void IEC_waterdata_EN_6BH(CPacketFrameFormat* appFrame);		//查询遥测站温度量数据上下限值 //add by yhp20160614
   //68H-7FH 备用
...
public:
   int  SendRtuLinkFrame_TmInit(BYTE* sBuf, int Len);
