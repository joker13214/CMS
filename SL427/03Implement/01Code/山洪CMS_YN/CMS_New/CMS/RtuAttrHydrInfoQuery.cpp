// RtuAttrHydrInfoQuery.cpp : 实现文件
//

#include "stdafx.h"
#include "CMS.h"
#include "RtuAttrHydrInfoQuery.h"


// CRtuAttrHydrInfoQuery 对话框
#define  W_TIMER_YCQUERY_CALL  0x01		//遥测查询选项查询Timer
#define  W_TIMER_YCQUERY_SET   0x02		//遥测查询选项设置Timer

IMPLEMENT_DYNAMIC(CRtuAttrHydrInfoQuery, CDialog)

CRtuAttrHydrInfoQuery::CRtuAttrHydrInfoQuery(CWnd* pParent /*=NULL*/)
	: CDialog(CRtuAttrHydrInfoQuery::IDD, pParent)
{
	m_pParentDlg = NULL;
	m_pSubStation = NULL;
}

CRtuAttrHydrInfoQuery::~CRtuAttrHydrInfoQuery()
{
}

void CRtuAttrHydrInfoQuery::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CRtuAttrHydrInfoQuery, CDialog)
	ON_BN_CLICKED(IDC_YCQUERY_CALL, &CRtuAttrHydrInfoQuery::OnBnClickedYcqueryCall)
	ON_BN_CLICKED(IDC_YCQUERY_SET, &CRtuAttrHydrInfoQuery::OnBnClickedYcquerySet)
	ON_NOTIFY(GVN_ENDLABELEDIT,IDC_RTU_YCQUERYGRIDCTRL,&CRtuAttrHydrInfoQuery::OnGridEndEdit)
	ON_WM_TIMER()
END_MESSAGE_MAP()


// CRtuAttrHydrInfoQuery 消息处理程序

BOOL CRtuAttrHydrInfoQuery::OnInitDialog()
{
...
void CRtuAttrHydrInfoQuery::InitGridCtrl()
{
...
void CRtuAttrHydrInfoQuery::OnCellNormal(int Row,int Col)
{
	if (m_wndGridCtrl.GetSafeHwnd())
	{
...
void CRtuAttrHydrInfoQuery::OnCellReadOnly(int Row,int Col)
{
	if (m_wndGridCtrl.GetSafeHwnd())
		m_wndGridCtrl.SetItemState(Row,Col, m_wndGridCtrl.GetItemState(Row,Col) | GVIS_READONLY);
...
CString CRtuAttrHydrInfoQuery::GetCellText(int Row,int Col)
{
	CString strText = "";
	if(m_wndGridCtrl.GetSafeHwnd())
	{
...
void CRtuAttrHydrInfoQuery::InitSetYcQuery(int Row,int Col)
{
	if (m_wndGridCtrl.GetSafeHwnd())
	{
...
void CRtuAttrHydrInfoQuery::SetParentDlg(CRtuPropertyDlg* pDlg)
{
	m_pParentDlg = pDlg;	
}


void CRtuAttrHydrInfoQuery::Update()
{
...
void CRtuAttrHydrInfoQuery::OnGridEndEdit(NMHDR *pNotifyStruct, LRESULT* pResult)
{
...
void CRtuAttrHydrInfoQuery::OnBnClickedYcqueryCall()
{
	UpdateData(TRUE);
...
void CRtuAttrHydrInfoQuery::OnBnClickedYcquerySet()
{
	UpdateData(TRUE);
	if (m_pSubStation != NULL)
	{
...
void CRtuAttrHydrInfoQuery::OnTimer(UINT_PTR nIDEvent)
{
	CString strLog= "";
...
void CRtuAttrHydrInfoQuery::SetWndsEnable(BOOL blEnable /* = TRUE */)
{
...
void CRtuAttrHydrInfoQuery::UpdateDlg(int attrType,BOOL blResult)
{
	m_blResult = blResult;
...
void CRtuAttrHydrInfoQuery::KillAllTimers()
{
