...
BOOL CRealTimeDB::CreateTables()
{
	...
	try
	{
		...
		//rtuStation table
		_strSQL.Format("CREATE TABLE rtuStation (sName varchar(50),sID varchar(40),ProvinceCode BYTE,RegionCode BYTE,CountyCode BYTE,AddrA2 LONG,AddrA3 BYTE,CmType BYTE,Interface BYTE,ComPortA varchar(20),BaundA int,DataBitsA BYTE,StopBitsA BYTE,ParityA BYTE,IpAddrA varchar(20),SockPortA int,AppProtoType BYTE,SCSW008StCode varchar(20),Centercode int ,SYQStCode LONG,PassWord int)");
		_LogMessage.Format("配置数据库操作,SQL:%s",_strSQL);
		InsertLog(SYS_OTHER_RUN_LOG,_LogMessage,Normal_Log_Message);
		if (m_Database.IsOpen())
			m_Database.ExecuteSQL(_strSQL);
...
BOOL CRealTimeDB::BuildRtuStations(CDatabase* pDb)
{
	...
	try
	{
		...
		sSQL.Format("SELECT DISTINCT sName,sID, ProvinceCode,RegionCode,CountyCode,AddrA2,AddrA3,CmType,Interface,ComPortA,BaundA,DataBitsA,StopBitsA,ParityA,IpAddrA,SockPortA, AppProtoType, SCSW008StCode,CenterCode,SYQStCode,PassWord FROM rtuStation ORDER BY ProvinceCode,RegionCode,CountyCode,AddrA2 ASC");
		...
		while (!rs.IsEOF())
		{
			...
			rs.GetFieldValue("AppProtoType", sTemp);
			info._bAppProType = atoi(sTemp);

			rs.GetFieldValue("SCSW008StCode", sTemp);
			info._strStCode = sTemp;

			rs.GetFieldValue("CenterCode", sTemp);
			info._centercode = atoi(sTemp);

			rs.GetFieldValue("SYQStCode", sTemp);
			info._strSYQADDR = (DWORD)atoi(sTemp);


			rs.GetFieldValue("PassWord", sTemp);
			info._passWord =atoi(sTemp);
...
// Access:    public 
// Returns:   CRtuStation*
// Qualifier: 通过stcode和协议类型查找Rtu站对象
// Parameter: DWORD stcode,int AppProtoType
//************
CRtuStation*  CRealTimeDB::SearchRtuStation(DWORD stcode,int AppProtoType)
{
	...
	while (rPos != NULL)
	{
		CRtuStation* tmpObj = (CRtuStation*)m_lstRtuStations.GetNext(rPos);

		if (tmpObj)
		{
			if (stcode == tmpObj->GetSYQStCode())
			{
				pObj = tmpObj;
				break;
			}
...
//要求终端机地址在所有站当中必须唯一编码
CRtuStation* CRealTimeDB::SearchRtuStation(CString strStCode, int AppProtoType)
{
	if (AppProtoType != SCSW_008_Protocol)
	{
		return NULL;
	}
	...
	while (rPos != NULL)
	{
		...
		if (tmpObj)
		{
			if (strStCode == tmpObj->GetStringStCode())
			{
				pObj = tmpObj;
				break;
			}
...
CRtuStation* CRealTimeDB::SearchRtuStation(WORD A2)
{
	CRtuStation *pObj = NULL;
...
bool CRealTimeDB::AddRtuStInfo(PRTU_STATION pInfo, CString& r_rtuID)
{
   ...
   try
   {
	   ...
	   strSQL.Format("INSERT INTO rtuStation (sName,sID, ProvinceCode,RegionCode,CountyCode,AddrA2,AddrA3,CmType,Interface,ComPortA,BaundA,DataBitsA,StopBitsA,ParityA,IpAddrA,SockPortA,AppProtoType,SCSW008StCode,CenterCode,SYQStCode) VALUES ('%s','%s',%d,%d,%d,%d,%d,%d,%d,'%s',%d,%d,%d,%d,'%s',%d, %d, '%s',%d,'%s')",
					 pInfo->_strName,
					 strID,
...
bool CRealTimeDB::CheckRtuStExist(CString strStCode)
{
	...
	while (rPos != NULL)
	{
		...
		if (pStation && pStation->GetStringStCode()==strStCode)
		{
			return true;
		}
	}
...
bool CRealTimeDB::CheckRtuStExist(CString strStCode, CRtuStation *pCurStation)
{
	if (m_lstRtuStations.IsEmpty())
		return false;
	...
	while (rPos != NULL)
	{
		...
		if (pStation && 
			...
			pStation->GetStringStCode() == strStCode)
		{
			return true;
		}
	}
...
//水雨情
bool CRealTimeDB::CheckCenterStNo(int centercode, CRtuStation *pCurStation)
{
	if (m_lstRtuStations.IsEmpty())
		return false;
	...
	{
		CRtuStation* pStation = (CRtuStation*)m_lstRtuStations.GetNext(rPos);
		if(pStation && pStation != pCurStation&&pStation->GetAppProtocolType()==SYQYC_Comm_Protocol)
		{
			if(centercode != pStation->GetSYQCenterCode())
				return true;
		}
...
//水雨情
bool CRealTimeDB::CheckSYQADDExist(int strAddr)//水雨情
{
	if (m_lstRtuStations.IsEmpty())
		return false;
	...
	{
		CRtuStation* pStation = (CRtuStation*)m_lstRtuStations.GetNext(rPos);
		if (pStation && pStation->GetSYQStCode()==strAddr)
		{
			return true;
		}
	}
...
//水雨情
bool CRealTimeDB::CheckSYQADDExist(int strAddr, CRtuStation *pCurStation)//水雨情
{
	if (m_lstRtuStations.IsEmpty())
		return false;
	...
	{
		CRtuStation* pStation = (CRtuStation*)m_lstRtuStations.GetNext(rPos);
		if (pStation&& pStation != pCurStation && pStation->GetSYQStCode()==strAddr)
		{
			return true;
		}
	}
...
bool CRealTimeDB::ModifyStInfo(PRTU_STATION pInfo, CRtuStation *pStation)
{
	...
	try
	{
		...
		strSQL.Format("UPDATE rtuStation SET sName ='%s',ProvinceCode=%d,RegionCode=%d,CountyCode=%d,AddrA2=%d,AddrA3=%d,CmType=%d,Interface=%d,ComPortA='%s',BaundA=%d,DataBitsA=%d,StopBitsA=%d,ParityA=%d,IpAddrA='%s',SockPortA=%d, AppProtoType=%d, SCSW008StCode='%s',CenterCode=%d,SYQStCode='%d',PassWord = '%d' WHERE sID='%s'",
			pInfo->_strName,
			pInfo->_bProvinceCode,
			pInfo->_bRegionCode,
			pInfo->_bCountyCode,
...
CRtuStation*  CRealTimeDB::QueryRtuInDb(CString rtuID)       //从数据库查找RTU站信息
{
	...
	try
	{
		sSQL.Format("SELECT DISTINCT sName,sID, ProvinceCode,RegionCode,CountyCode,AddrA2,AddrA3,CmType,Interface,ComPortA,BaundA,DataBitsA,StopBitsA,ParityA,IpAddrA,SockPortA,AppProtoType, SCSW008StCode,CenterCode,SYQStCode FROM rtuStation WHERE sID='%s'",rtuID);
		sLogMessage.Format("配置数据库操作,SQL:%s",sSQL);
		InsertLog(SYS_OTHER_RUN_LOG,sLogMessage,Normal_Log_Message);
		...
		if (!rs.IsEOF())
		{
			...
			rs.GetFieldValue("SYQStCode", sTemp);
			info._strSYQADDR = (BYTE)atoi(sTemp);
	...
	try
	{
		while (rPos != NULL)
		{
			...
			if (pObj)
			{
				...
				info._bAppProType = pObj->GetAppProtocolType();
				info._strStCode = pObj->GetStringStCode();
				info._centercode = pObj->GetSYQCenterCode();
				info._strSYQADDR = pObj->GetSYQStCode();
				info._passWord = pObj->GetSYQPassword();
