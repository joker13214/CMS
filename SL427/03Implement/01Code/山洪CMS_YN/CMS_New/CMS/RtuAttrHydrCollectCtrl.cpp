// RtuAttrCollectCtrl.cpp : 实现文件
//

#include "stdafx.h"
#include "CMS.h"
#include "RtuAttrHydrCollectCtrl.h"


// CRtuAttrCollectCtrl 对话框
#define  W_TIMER_YCQUERY_CALL  0x01		//遥测查询选项查询Timer
#define  W_TIMER_YCQUERY_SET   0x02		//遥测查询选项设置Timer
IMPLEMENT_DYNAMIC(CRtuAttrHydrCollectCtrl, CDialog)

CRtuAttrHydrCollectCtrl::CRtuAttrHydrCollectCtrl(CWnd* pParent /*=NULL*/)
	: CDialog(CRtuAttrHydrCollectCtrl::IDD, pParent)
{

}

CRtuAttrHydrCollectCtrl::~CRtuAttrHydrCollectCtrl()
{
}

void CRtuAttrHydrCollectCtrl::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CRtuAttrHydrCollectCtrl, CDialog)
	ON_BN_CLICKED(IDC_YCQUERY_CALL, &CRtuAttrHydrCollectCtrl::OnBnClickedYcqueryCall)
	ON_BN_CLICKED(IDC_YCQUERY_SET, &CRtuAttrHydrCollectCtrl::OnBnClickedYcquerySet)
	ON_NOTIFY(GVN_ENDLABELEDIT,IDC_RTU_YCQUERYGRIDCTRL,&CRtuAttrHydrCollectCtrl::OnGridEndEdit)
	ON_WM_TIMER()
END_MESSAGE_MAP()


// CRtuAttrCollectCtrl 消息处理程序

BOOL CRtuAttrHydrCollectCtrl::OnInitDialog()
{
...
void CRtuAttrHydrCollectCtrl::InitGridCtrl()
{
...
void CRtuAttrHydrCollectCtrl::OnCellNormal(int Row,int Col)
{
	if (m_wndGridCtrl.GetSafeHwnd())
	{
...
void CRtuAttrHydrCollectCtrl::OnCellReadOnly(int Row,int Col)
{
	if (m_wndGridCtrl.GetSafeHwnd())
		m_wndGridCtrl.SetItemState(Row,Col, m_wndGridCtrl.GetItemState(Row,Col) | GVIS_READONLY);
...
CString CRtuAttrHydrCollectCtrl::GetCellText(int Row,int Col)
{
	CString strText = "";
	if(m_wndGridCtrl.GetSafeHwnd())
	{
...
void CRtuAttrHydrCollectCtrl::InitSetYcQuery(int Row,int Col)
{
	if (m_wndGridCtrl.GetSafeHwnd())
	{
...
void CRtuAttrHydrCollectCtrl::SetParentDlg(CRtuPropertyDlg* pDlg)
{
	m_pParentDlg = pDlg;	
}


void CRtuAttrHydrCollectCtrl::Update()
{
...
void CRtuAttrHydrCollectCtrl::OnGridEndEdit(NMHDR *pNotifyStruct, LRESULT* pResult)
{
...
void CRtuAttrHydrCollectCtrl::OnBnClickedYcqueryCall()
{
	UpdateData(TRUE);
...
void CRtuAttrHydrCollectCtrl::OnBnClickedYcquerySet()
{
	UpdateData(TRUE);
	if (m_pSubStation != NULL)
	{
...
void CRtuAttrHydrCollectCtrl::OnTimer(UINT_PTR nIDEvent)
{
	CString strLog= "";
...
void CRtuAttrHydrCollectCtrl::SetWndsEnable(BOOL blEnable /* = TRUE */)
{
...
void CRtuAttrHydrCollectCtrl::UpdateDlg(int attrType,BOOL blResult)
{
	m_blResult = blResult;
...
void CRtuAttrHydrCollectCtrl::KillAllTimers()
{
