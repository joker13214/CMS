...
void CCommThreadProDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CBX_APPPROTOTYPE, m_wndAppProtoType);
}
...
BOOL CCommThreadProDlg::OnInitDialog()
{
	...
	if (m_wndAppProtoType.GetSafeHwnd())
	{
		m_wndAppProtoType.ResetContent();
		int appType_idx = 0;
		appType_idx = m_wndAppProtoType.AddString("SL427规约");
		m_wndAppProtoType.SetItemData(appType_idx, (BYTE)WaterResources_Protocol);

		appType_idx = m_wndAppProtoType.AddString("SCSW008规约");
		m_wndAppProtoType.SetItemData(appType_idx, (BYTE)SCSW_008_Protocol);
		//水雨情遥测系统数据通信规约
		appType_idx = m_wndAppProtoType.AddString("水雨情遥测系统数据通信规约");
		m_wndAppProtoType.SetItemData(appType_idx, (BYTE)SYQYC_Comm_Protocol);

		if (m_wndAppProtoType.GetCount())
		{
			if (m_pCurCommThreadInfo)
			{
				for (int i=0; i< m_wndAppProtoType.GetCount(); i++)
				{
					if (m_wndAppProtoType.GetItemData(i) == m_pCurCommThreadInfo->GetAppProtoType())
					{
						m_wndAppProtoType.SetCurSel(i);
						break;
					}
				}
			}
			else 
				m_wndAppProtoType.SetCurSel(0);
		}
	}
	...
	if ( m_wndCmbCmType.GetSafeHwnd())
	{
		...
		*/
		...
		m_wndCmbCmType.SetItemData(idx,GSM_CommChannel);
		*/
	...
	if (m_wndCmbSccj.GetSafeHwnd())
	{
		...
		int Idx = 0;
		Idx = m_wndCmbSccj.AddString("国腾电子");
		m_wndCmbSccj.SetItemData(Idx,SCFactoryType_GTDZ);

		Idx = m_wndCmbSccj.AddString("神州天鸿");
		m_wndCmbSccj.SetItemData(Idx,SCFactoryType_SZTH);
...
void CCommThreadProDlg::UpdateThreadInfo()
{
	...
	if (m_pCurCommThreadInfo)
	{
		...
		m_bAppProtoType = m_pCurCommThreadInfo->GetAppProtoType();
...
void CCommThreadProDlg::OnOK()
{
	...
	int nSel_AppProtoType = m_wndAppProtoType.GetCurSel();
	if (nSel_AppProtoType == CB_ERR)
	{
		MessageBox("请选择应用层规约类型!","错误",MB_OK|MB_ICONERROR);
		return;
	}
	...
	m_bAppProtoType = (BYTE)m_wndAppProtoType.GetItemData(nSel_AppProtoType);
