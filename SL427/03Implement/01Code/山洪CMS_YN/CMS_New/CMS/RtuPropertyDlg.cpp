	...
	: CDialog(CRtuPropertyDlg::IDD, pParent)
{
	...
	m_pTabHydrProSet = NULL;
	m_pTabWriteUpdate = NULL;
	m_pTabSyqProSet = NULL;
}
...
BOOL CRtuPropertyDlg::OnInitDialog()
{
	...
	m_pTabSyqProSet = new CRtuAttrSyqProSet(&m_wndTabCtrl);
	m_pTabSyqProSet->SetParentDlg(this);
	m_pTabSyqProSet->Create(CRtuAttrSyqProSet::IDD, &m_wndTabCtrl);
	m_wndTabCtrl.AddTab(m_pTabSyqProSet,"水雨情系统设置/查询");

	m_pTabHydrProSet = new CRtuAttrHydrProSet(&m_wndTabCtrl);
	m_pTabHydrProSet->SetParentDlg(this);
	m_pTabHydrProSet->Create(CRtuAttrHydrProSet::IDD, &m_wndTabCtrl);
	m_wndTabCtrl.AddTab(m_pTabHydrProSet,"SCSW008参数设置/查询");
	...
	m_wndTabCtrl.AddTab(m_pTabAttrProSet,"SL427参数设置/查询");
	...
	m_pTabWriteUpdate = new CRtuAttrWriteUpdate(&m_wndTabCtrl);
	m_pTabWriteUpdate->SetParentDlg(this);
	m_pTabWriteUpdate->Create(CRtuAttrWriteUpdate::IDD,&m_wndTabCtrl);
	m_wndTabCtrl.AddTab(m_pTabWriteUpdate,"烧写升级");


	

// 	int nType = WaterResources_Protocol;
// 	if (m_pCurRtuStation != NULL)
// 	{
// 		nType = m_pCurRtuStation->GetAppProtocolType();
// 	}
// 
// 	switch (nType)
// 	{
// 	case SCSW_008_Protocol:
// 		m_pTabHydrProSet->ShowWindow(SW_SHOW);
// 		m_pTabAttrProSet->ShowWindow(SW_SHOW);
// 		break;
// 	case WaterResources_Protocol:
// 	case Hydrology_Protocol:
// 		m_pTabHydrProSet->ShowWindow(SW_SHOW);
// 		m_pTabAttrProSet->ShowWindow(SW_SHOW);
// 		break;
// 	}
...
void CRtuPropertyDlg::Update()
{
	...
	switch (m_pCurRtuStation->GetAppProtocolType())
	{
	case WaterResources_Protocol:
	case Hydrology_Protocol:
		if (m_pTabAttrProSet && m_pTabAttrProSet->GetSafeHwnd())
		{
			m_pTabAttrProSet->EnableWindow(TRUE);
			m_pTabAttrControlCmd->EnableWindow(TRUE);
			m_pTabAttrProSet->EnableWindow(TRUE);
			m_pTabHydrProSet->EnableWindow(FALSE);
 			m_pTabAttrProSet->Update();
		}
		break;
	case SCSW_008_Protocol:
		if (m_pTabHydrProSet && m_pTabHydrProSet->GetSafeHwnd())
		{
			m_pTabAttrProSet->EnableWindow(FALSE);
			m_pTabAttrControlCmd->EnableWindow(FALSE);
			m_pTabAttrProSet->EnableWindow(FALSE);
			m_pTabHydrProSet->EnableWindow(TRUE);
 			m_pTabHydrProSet->Update();
		}
	case  SYQYC_Comm_Protocol://zwb
		if(m_pTabSyqProSet&&m_pTabSyqProSet->GetSafeHwnd())
		{
			m_pTabAttrProSet->EnableWindow(TRUE);
			m_pTabSyqProSet->Update();
		}
	...
	if (m_pTabWriteUpdate && m_pTabWriteUpdate->GetSafeHwnd())
		m_pTabWriteUpdate->Update();
