...
int CUdfLogSystem::QueryLogInformation(_LOG_MESSAGE_TYPE _nType,CUdfLogQueryDlg::QUERY_LOG_TYPE queryLogType/* =CUdfLogQueryDlg::QUERY_LOGTYPE_ALL */,PSYSTEMTIME _beginTime/* =NULL */, PSYSTEMTIME _endTime /* = NULL */)
{
	...
	try
	{
		...
		if (_beginTime ==  NULL || _endTime == NULL)
		{
			...
			if (strWhereLogType.IsEmpty())
				_sSQL.Format("SELECT TOP 5000 IDX,recordtime,logtype,xlevel,messages FROM %s ORDER BY IDX DESC",_sTableName);
			else
				_sSQL.Format("SELECT TOP 5000 IDX,recordtime,logtype,xlevel,messages FROM %s WHERE %s ORDER BY IDX DESC",_sTableName,strWhereLogType);
		}
		else
		{
			CString _strBeginTime="",_strEndTime="";
			...
			//	_strEndTime);
			if (strWhereLogType.IsEmpty())
				_sSQL.Format("SELECT TOP 5000 recordtime,logtype,xlevel,messages FROM %s  WHERE recordtime between #%s# and #%s# ORDER BY recordtime DESC",
				_sTableName,
				_strBeginTime,
				_strEndTime);
			else
				_sSQL.Format("SELECT  TOP 5000 recordtime,logtype,xlevel,messages FROM %s  WHERE %s AND recordtime BETWEEN #%s# AND #%s# ORDER BY recordtime DESC",
				_sTableName,
				strWhereLogType,
				_strBeginTime,
				_strEndTime);
