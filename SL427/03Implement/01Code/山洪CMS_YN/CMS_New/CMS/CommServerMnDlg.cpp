...
void CCommServerMnDlg::ShowOneThreadInfo(CCommThreadInfo* pThread,int ItemRow)
{
	...
	{
		CString str1;
		if(pThread->GetAppProtoType()==1)
			str1 ="SL427规约";
		else if(pThread->GetAppProtoType()==3)
			str1 = "SCSW008规约";
		else if(pThread->GetAppProtoType()==4)
			str1 = "水雨情遥测系统数据通信规约";

		strTemp.Format("协议类型:%s,应用层协议类型:%s,服务IP和端口(%s,%d)",
...
void CCommServerMnDlg::OnBnClickedButton1()
{
	...
	if (dlg.DoModal()== IDOK)
	{
		...
		if (pThreadNew)
		{
			...
			pThreadNew->SetAppProtoType(dlg.m_bAppProtoType);
		...
		if (pThread->GetCommType() != dlg.m_bCom_Type)
		{
				...
				dlg.m_bCom_Type == SC_CommChannel))
			{
				if (pCommServer->StopCommThread(pThread) == true)
				{
					...
					pThread->SetAppProtoType(dlg.m_bAppProtoType);
