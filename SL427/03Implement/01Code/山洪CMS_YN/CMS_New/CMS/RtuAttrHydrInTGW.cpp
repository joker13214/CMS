...
void CRtuAttrHydrInTGW::UpdateDlg(int attrType,BOOL blResult)			//接收到报文后、更新界面
{
	m_blResult = blResult;
...
void CRtuAttrHydrInTGW::OnTimer(UINT_PTR nIDEvent)
{
	...
	case W_TIMER_WATERLEVEL_REAL_QUERY:
		{
			if (m_blResult == TRUE)
				strLogMessge = "查询遥测终端的地下水瞬时温度值 成功!";
			else if( m_blResult == FALSE)
			{
				strLogMessge = "查询遥测终端的地下水瞬时温度值 超时!";
