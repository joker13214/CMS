	...
	*/
	...
	if (m_pParentDlg)
	{
		...
		if (pSub)
		{
				...
				*/
				...
				if (nAppProV == APP_PROTOCOL_V2_0)
				{
					ItemIdx = m_wndCmbWorkMode.AddString("间歇工作模式");
						m_wndCmbWorkMode.SetItemData(ItemIdx,4);
				}
			...
			if (m_pSubStation->GetAppProtocolType() == WaterResources_Protocol)
			{
				m_strA2.Format("%d",m_pSubStation->GetAddrA2());
			}
			else if (m_pSubStation->GetAppProtocolType() == SCSW_008_Protocol)
			{
				m_strA2 = m_pSubStation->GetStringStCode();
			}
			if (m_pSubStation->GetAppProtocolType() == SYQYC_Comm_Protocol)//zwb >????
			{
				m_strA2.Format("%d",m_pSubStation->GetSYQStCode());
			}		
...
void CRtuAttrBasicInfo::OnBnClickedConnCheck()//链路测试
{
	...
	if( m_pSubStation)
	{
		if (m_pSubStation->blEnbaleTrans() == true)
		{
			...
			else if(m_pSubStation->GetAppProtocolType()== SYQYC_Comm_Protocol)
			{
				CSYQFrameFormat appFrame ;
				m_pSubStation->BuildBasicPacket(appFrame);

				m_pSubStation->GetSysDataTrans_Syq()->SYQ_EN_COMMON(&appFrame,0x30,TRUE);
			}
