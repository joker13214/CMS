...
public:
	...
	void SetAppProtoType(BYTE b)                {m_AppProtoType = b;}
	...
	const BYTE GetAppProtoType()                {return m_AppProtoType;}
...
public:
	...
	void            FrameBufDeal_SCSW008(BYTE* buf,int nLen,int FrameType, BYTE CmInterface=Serial_CommInterface,
										 CString FromIP="",int FromPort=0);	//数据报文帧处理

	void            FrameBufDeal_SYQ(BYTE* buf,int nLen,int FrameType, BYTE CmInterface=Serial_CommInterface,
									CString FromIP="",int FromPort=0);//数据报文帧处理
