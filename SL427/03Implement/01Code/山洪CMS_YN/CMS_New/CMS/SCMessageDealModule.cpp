...
CSCMessageDealModule::CSCMessageDealModule( UINT rAddr,CScModuleBase* pScModule )
{
	...
	m_pBase = new CProtocol_ComScHandler();
	m_pHandler_SCSW008 = new CSCSW008_ComScHandler();
	m_pHandler_SL427 = new CSl427_ComScHandler();
	m_pBase->SetParent(this);
	m_pHandler_SCSW008->SetParent(this);
	m_pHandler_SL427->SetParent(this);

	m_pBase = m_pHandler_SL427;
...
void CSCMessageDealModule::SwitchHandlerModel(int AppProtoType)
{
	switch (AppProtoType)
	{
...
void CSCMessageDealModule::RecvMessage( BYTE* buf,int Len )
{
	if (buf && Len > 0)
	{
		...
		m_pBase->CopyBuf(m_RECV_BUF, m_RECV_Len);
		m_pBase->DealMessage();
	}
}
...
bool CSCMessageDealModule::SendMessage( BYTE* buf,int Len )
{
	if (buf != NULL && Len > 0)
	{
		if (GetScModule() != NULL )
		{
			...
			else if (scType == SCFactoryType_SZTH)
			{
				CScModule_SZTH* pScModuleSZTH = (CScModule_SZTH*)GetScModule();
				if (pScModuleSZTH)
				{
					pScModuleSZTH->SndDown_TXSQMessage(70,m_nRemoteAddr,0x00,buf,Len);
					return true;
				}
			}
		}
